...
function crearTablero() {
    ...
    for (let i = 0; i < 30; i++) {
        ...
        for (let j = 0; j < 30; j++) {
            ...
            td.addEventListener("mousedown", pintar);
            td.addEventListener("mousemove", pintarArrastrando);
            td.addEventListener("mouseup", parar);
...
//FUNCIÓN PINTAR
//El color del pincel será el que se seleccione
function pintar() {
        this.className = pincel;   
}

function pintarArrastrando(e) {
    if (e.buttons === 1){
        this.className = pincel; 
    } else if (e.buttons === 0) {
...
//FUNCIÓN PARAR
//Al levantar el pincel se queda en blanco
function parar() {
    colorSel = btnBlanco;
    pincel = colorSel.classList.remove("seleccionado");
    pincel = colorSel.className;
    this.className = pincel;
    console.log(pincel);
}