...
let milisegundos = finCurso.valueOf() - diaActual.valueOf();
...
//Apartado 2
let diaCumple:number = Number(prompt("Introduce o dia do teu cumpleanos"));
let mesCumple:number = Number(prompt("Introduce o mes do teu cumpleanos"));
let ano = new Date().getFullYear();

for (ano; ano < 2100; ano++) {
    let diaSemana:Date = new Date(ano, Number(mesCumple)-1, Number(diaCumple));
    diaSemana.getDay();
